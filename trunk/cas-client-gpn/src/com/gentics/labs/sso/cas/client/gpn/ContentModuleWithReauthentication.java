...
public class ContentModuleWithReauthentication extends GenticsContentModule {
	...
	protected void doView(RenderRequest req, RenderResponse res)
		...
		if (SHOWPASSWORDFIELD_CMD.equals(req.getParameter("cmd"))) {
			String casServerUrlPrefix = getGenticsPortletContext()
					.getStringModuleParameter("casServerUrlPrefix");
			...
			ReauthenticationHelper.getInstance(casServerUrlPrefix)
					.reauthenticateOrShowPasswordField(req, res,
			...
			String redirectUrl = ObjectTransformer.getString(session
					.getAttribute(REAUTH_URL_PARAM,
							PortletSession.PORTLET_SCOPE), null);
			String redirectName = ObjectTransformer.getString(session
					.getAttribute(REAUTH_NAME_PARAM,
							PortletSession.PORTLET_SCOPE), null);
			if (redirectUrl != null) {
				if (logger.isDebugEnabled()) {
					logger.debug("Redirecting to the resource (should be authenticated now)");
				}
				session.removeAttribute(REAUTH_URL_PARAM,
						PortletSession.PORTLET_SCOPE);

				PortletURL renderURL = res.createRenderURL();
				// show the download link
				res.getWriter().println(
						"Download " + redirectName + " <a href=\""
								+ StringUtils.encodeWithEntities(redirectUrl)
								+ "\">here</a><br/>Go <a href=\"" + renderURL
								+ "\">back</a>");
				return;
			}

			if (ObjectTransformer.getBoolean(
					getProperty(getReauthAttributeName()), false)) {
				if (logger.isDebugEnabled()) {
					logger.debug("Accessed content (" + getProperty("contentid")
				...
				if (!ReauthenticationHelper.getInstance(casServerUrlPrefix)
					...
					if (logger.isDebugEnabled()) {
						logger
								.debug("Showing password field to reauthenticate for content {"
					...
					if (logger.isDebugEnabled()) {
						logger
								.debug("User successfully reauthenticated for content {"
										+ getProperty("contentid") + "}");
					}
				}
			}
			...
			if (logger.isDebugEnabled()) {
				logger.debug("Rendering content {" + getProperty("contentid")
						+ "} now");
			}
	...
	public void serveResource(ResourceRequest request, ResourceResponse response)
		...
		try {
			...
			if (obj != null
					&& ObjectTransformer.getBoolean(obj
							.get(getReauthAttributeName()), false)) {
				String resourceName = ObjectTransformer.getString(obj.get("name"), null);
				if (logger.isDebugEnabled()) {
					logger.debug("Accessed resource "
							+ obj.getContentId()
				...
				String casServerUrlPrefix = getGenticsPortletContext()
						.getStringModuleParameter("casServerUrlPrefix");
				if (ReauthenticationHelper.getInstance(casServerUrlPrefix)
						.reauthenticateOrRedirect(request, response,
								getGenticsPortletContext().getModuleId() + "_r",
								renderUrl.toString())) {
					if (logger.isDebugEnabled()) {
						logger.debug("User successfully reauthenticated for resource {"+obj.getContentId()+"}");
					}
					writeResourceIntoResponse(request, response, obj, false);
				} else {
					if (logger.isDebugEnabled()) {
						logger
								.debug("Request will be redirected to the password form");
					}
				}
			} else {
	...
	protected String getReauthAttributeName() {
		return ObjectTransformer.getString(getGenticsPortletContext()
				.getStringModuleParameter("reauthentication_attribute_name"),
